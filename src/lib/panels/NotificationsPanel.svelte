<script lang="ts">
  ...
  interface Notif {
    ...
    body: string;
    source: string;
    at: string; // relative-ish timestamp string
  }
  ...
  const notifs: Notif[] = [
    ...
    { kind: "message", title: "Calendar", body: "Standup in 15 minutes.", source: "Google", at: "3h ago" },
  ];

  function color(k: Kind): string {
    switch (k) {
      case "warning":
        return "#FFB454";
      case "success":
...
<section class="panel flex h-full w-full flex-col justify-center px-16">
  <div class="mb-3 flex items-center gap-2 text-quake">
    <Bell size={22} />
    <span class="text-lg font-medium text-white">Notifications</span>
    <span class="text-xs text-white/40">· via OpenClaw</span>
  </div>

  <div class="notif-scroll flex flex-col gap-2 overflow-y-auto pr-2">
    {#each notifs as n, i (i)}
      {@const Icon = ICON[n.kind]}
      <div class="notif flex items-start gap-3 rounded-lg bg-ink-800/50 px-4 py-2.5">
        <Icon size={20} style={`color:${color(n.kind)}`} class="mt-0.5 shrink-0" />
        <div class="min-w-0 flex-1">
          <div class="flex items-baseline justify-between gap-3">
            <span class="truncate text-base font-medium text-white">{n.title}</span>
            <span class="shrink-0 text-xs text-white/40">{n.at} · {n.source}</span>
          </div>
          <p class="truncate text-sm text-white/60">{n.body}</p>
        </div>
      </div>
    {/each}
  </div>
